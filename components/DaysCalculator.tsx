...
export default function DaysCalculator() {
  ...
  const formatDateRu = (d?: Date | null) =>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 border-2 border-neutral-300 shadow-sm">
        <h3
          className="text-base font-bold text-neutral-900 mb-4"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={useRuHolidays}
              onChange={(e) => setUseRuHolidays(e.target.checked)}
            />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            ...
            <div className="bg-neutral-50 p-4 col-span-2 md:col-span-4">
              ...
              <div className="text-lg font-semibold">
                {diff.ymd.years} лет, {diff.ymd.months} мес., {diff.ymd.days}{" "}
                дн.
              </div>
            </div>
          </div>
      ...
      <div className="bg-white p-6 border-2 border-neutral-300 shadow-sm">
        ...
        {adjusted && (
          <div className="mt-4 text-sm text-neutral-700">
            Результат:{" "}
            <span className="font-semibold">{formatDateRu(adjusted)}</span>
            <div className="text-xs text-neutral-500 mt-1">
              Формула: {formatDateRu(base)} {mode === "add" ? "+" : "−"}{" "}
              {parseInt(years || "0", 10) || 0} лет,{" "}
              {parseInt(months || "0", 10) || 0} мес.,{" "}
              {parseInt(days || "0", 10) || 0} дн.
            </div>
          </div>
        )}
      </div>
