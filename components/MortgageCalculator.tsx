...
import {
  calculateAnnuityDetails,
  calculateDifferentiatedDetails,
  type MortgageDetailedResult,
} from "../logic/mortgage";
...
const inputClass =
  "w-full px-4 py-3 text-base bg-white border-2 border-neutral-300 focus:outline-none focus:border-neutral-900";
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-4 sm:gap-6">
        <div className="flex items-center gap-4">
          <button
            className={`text-sm ${mode === "byPrice" ? "font-semibold border-b-2 border-neutral-900 text-neutral-900" : "text-neutral-600"}`}
            onClick={() => setMode("byPrice")}
          >
            По стоимости
          </button>
          <button
            className={`text-sm ${mode === "byLoan" ? "font-semibold border-b-2 border-neutral-900 text-neutral-900" : "text-neutral-600"}`}
            onClick={() => setMode("byLoan")}
          >
            По сумме кредита
          </button>
        </div>
        ...
        {mode === "byPrice" ? (
          <>
            ...
            <div>
              ...
              <NumericInput
                ...
                onChange={handleInput(setInitialPayment)}
                className={inputClass}
          ...
          <div>
            ...
            <NumericInput
              ...
              onChange={handleInput(setLoanAmountInput)}
              className={inputClass}
        ...
        <div className="space-y-6">
          <div className="bg-neutral-50 p-6 border-2 border-neutral-300">
            ...
            <div className="text-4xl md:text-5xl font-bold text-neutral-900">
              {formatInt(details.monthlyPayment)} ₽
            </div>
            {paymentType === "differentiated" && (
              <div className="text-xs text-neutral-600 mt-1">
                Первый платёж: {formatInt(details.firstPayment)} ₽, последний:{" "}
                {formatInt(details.lastPayment)} ₽
              </div>
            )}
          </div>
          ...
          <div className="bg-neutral-50 p-5 border-2 border-neutral-300">
            ...
            <div className="text-2xl font-bold">
              {formatInt(details.totalPayment)} ₽
            </div>
          </div>
          ...
          <div className="pt-6 mt-6 border-t-2 border-neutral-300">
            ...
            <div className="hidden sm:block">
              ...
              <div className="max-h-64 overflow-auto border-2 border-neutral-300 bg-white">
                {details.schedule.map((row) => (
                  <div
                    key={row.month}
                    className="grid grid-cols-5 gap-2 px-3 py-2 text-sm border-b border-neutral-200"
                  >
                    <div>{row.month}</div>
            ...
            <div className="sm:hidden max-h-80 overflow-auto space-y-2">
              {details.schedule.slice(0, 24).map((row) => (
                <div
                  key={row.month}
                  className="bg-white border-2 border-neutral-300 p-3 text-sm"
                >
                  ...
                  <div className="grid grid-cols-2 gap-1 text-xs text-neutral-600">
                    <div>
                      ...
                      <span className="text-neutral-900">
                        {formatInt(row.payment)} ₽
                      </span>
                    </div>
                    <div>
                      Проценты:{" "}
                      <span className="text-neutral-900">
                        {formatInt(row.interest)} ₽
                      </span>
                    </div>
                    <div>
                      Основной:{" "}
                      <span className="text-neutral-900">
                        {formatInt(row.principal)} ₽
                      </span>
                    </div>
                    <div>
                      Остаток:{" "}
              ...
              {details.schedule.length > 24 && (
                <div className="text-xs text-neutral-500 text-center py-2">
                  Показаны первые 24 из {details.schedule.length} месяцев.
                  Полный график доступен на десктопе.
                </div>
              )}
            </div>
          </div>
