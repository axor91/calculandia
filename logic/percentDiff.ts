...
export function calculatePercentDifference(baseValue: number, nextValue: number): number | null {
  if (!Number.isFinite(baseValue) || !Number.isFinite(nextValue) || baseValue === 0) {
    return null;
  }
...
export function calculatePercentDifferenceAverage(value1: number, value2: number): number | null {
...
export function calculatePercentRatio(value1: number, value2: number): number | null {
  if (!Number.isFinite(value1) || !Number.isFinite(value2)) {
    return null;
  }
