...
describe("fractions logic", () => {
  describe("simplifyFraction", () => {
    ...
    it("returns NaN for non-finite inputs", () => {
      expect(
        simplifyFraction({ numerator: NaN, denominator: 1 }).numerator,
      ).toBeNaN();
      expect(
        simplifyFraction({ numerator: 1, denominator: Infinity }).denominator,
      ).toBeNaN();
    });
